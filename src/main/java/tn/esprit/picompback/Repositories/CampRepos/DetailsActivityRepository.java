...
public interface DetailsActivityRepository extends JpaRepository<DetailsActivity,Long> {
    ...
    @Query("SELECT COALESCE(max(da.idDA),0) FROM DetailsActivity da INNER JOIN da.activity activity WHERE da.activity.id_activity = activity.id_activity And da.activity.id_activity = :idActivity ")
